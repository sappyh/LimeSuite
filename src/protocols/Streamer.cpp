...
StreamChannel::StreamChannel(Streamer *streamer) : mActive(false)
{
    mStreamer = streamer;
    overflow = 0;
    underflow = 0;
...
void Streamer::ReceivePacketsLoop()
{
    ...
    while (terminateRx.load() == false)
    {
        ...
        for (uint8_t pktIndex = 0; pktIndex < bytesReceived / sizeof(FPGA_DataPacket); ++pktIndex)
        {
            ...
            // std::cout << " The timestamp is " << pkt[pktIndex].counter << std::endl;
            ...
            int samplesCount = FPGA::FPGAPacketPayload2Samples(pktStart, 3056, chCount == 2, packed, dest.data());
        ...
        // auto rssi = lms->GetRSSI();
        // auto finish = std::chrono::high_resolution_clock::now();

        // std::cout << "Time Taken: " << std::chrono::duration_cast<std::chrono::microseconds>(finish - start).count() << std::endl;
