...
int ConnectionFX3::Write(const unsigned char *buffer, const int length, int timeout_ms)
{
    ...
    bulkCtrlInProgress = false;
#ifndef __unix__
    if (bulkCtrlAvailable && commandsToBulkCtrl.find(buffer[0]) != commandsToBulkCtrl.end())
    {
...
void callback_libusbtransfer(libusb_transfer *trans)
{
    ...
    case LIBUSB_TRANSFER_COMPLETED:
        context->bytesXfered = trans->actual_length;
        context->done.store(true);
        // if(trans->endpoint==0x81)
        // {
        //     clock_gettime(CLOCK_REALTIME,&tnow2);
        //     fp2 << tnow2.tv_sec * 1000000 + tnow2.tv_nsec / 1000L << std::endl;
        // }
        break;
...
int ConnectionFX3::BeginDataSending(const char *buffer, uint32_t length, int ep)
{
    ...
    clock_gettime(CLOCK_REALTIME, &tnow1);
    fp1 << tnow1.tv_sec * 1000000 + tnow1.tv_nsec / 1000L << std::endl;
    int status = libusb_submit_transfer(tr);
    if (status != 0)
    {
